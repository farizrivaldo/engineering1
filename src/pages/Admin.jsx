...
function Admin() {
  ...
  const fetchUser = async () => {
    let response = await axios.get("http://10.126.15.138:8002/part/alluser");
  ...
  const updateUser = async (id, data) => {
    ...
    let response = await axios.patch(
      `http://10.126.15.138:8002/part/userupdate/${id}`,
      dataUser
    );
    if (response) {
  ...
  const deleteUser = async (id) => {
    let response = await axios.delete(
      `http://10.126.15.138:8002/part/userdelete/${id}`
    );
    if (response) {
      alert(response.data.message);
    }
  ...
  const editUser = async (id) => {
    let response = await axios.patch(
      `http://10.126.15.138:8002/part/useredit/${id}`
    );
